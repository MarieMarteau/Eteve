...
function init3DObjects(sceneGraph, pickingData) {
	...
	// Création du levier
	const CGeometry = new THREE.CylinderGeometry(0.6,0.6,0.4, 32, 1, false, 0, Math.PI);
	const cylindre = new THREE.Mesh( CGeometry, MaterialRGB(108/258,58/258,58/258));
	cylindre.position.set(-3,-1,0);
	cylindre.rotateZ(Math.PI/2);
	cylindre.rotateX(Math.PI/2);
	cylindre.name="cylindre"
	sceneGraph.add(cylindre);
	
	const LGeometry = new THREE.CylinderGeometry(0.05,0.05,1.5, 32);
	const levier = new THREE.Mesh( LGeometry, MaterialRGB(108/258,58/258,58/258));
	sceneGraph.add(levier);
	levier.name="levier";
	levier.position.set(-3,1.5/2-1,0);
	...
	sphere.position.set(-3,1.5-1,0);
	pickingData.selectableObjects.push(sphere);
...
function animate(sceneThreeJs, time,pickingData) {
	...
	const cylindre = sceneThreeJs.sceneGraph.getObjectByName("cylindre");
...
function initEmptyScene(sceneThreeJs) {
    ...
    sceneInit.insertLight(sceneThreeJs.sceneGraph,Vector3(-3,3,2));
